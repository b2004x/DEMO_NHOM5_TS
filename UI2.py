...
class ManualScaler:
    ...
    def inverse_transform(self, x_scaled):
        return np.array(x_scaled) * self.scale + self.min_val

    def transform(self, x):
        return (np.array(x) - self.min_val) / self.scale
...
if st.button("Tiếp tục"):
    ...
    target_scaler = ManualScaler(min_val=253, max_val=479)
    # ============== MODEL ==================
    # === Dataset ===
    ...
    elif Models_name == "gru":
        st.title("🔮 Dự đoán doanh số 7 ngày tới bằng mô hình GRU")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        model = GRUModel(input_size).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
        criterion = nn.MSELoss()
    elif Models_name == "transformer":
        device = torch.device("cpu")
        st.title("🔮 Dự đoán doanh số 7 ngày tới bằng mô hình Transformer")
    ...
    if Models_name == "transformer":
        model.load_state_dict(torch.load(rf"G:\hoc\Time series\UI\Models\{Models_name}_store{store_id}_family{family_id}.pth", map_location=device))
    # 2. Load trọng số từ file đã lưu
    else:
        model.load_state_dict(torch.load(rf"G:\hoc\Time series\UI\Models\{Models_name}_model_store{store_id}_family{family_id}.pth", map_location=device))
    ...
    multi_preds_rescaled = target_scaler.inverse_transform(multi_preds)

    multi_preds_rescaled = np.rint(multi_preds_rescaled).astype(int).tolist()

    true_last_30 = [row[0] for row in dataset.buffer[-30:]]
    true_last_30 = target_scaler.inverse_transform(true_last_30)
...
# Tạo thư mục lưu nếu chưa có
    ...
    save_dir = os.path.join(base_dir, f"results", f"store_{store_id}_family_{family_id}_Model_{Models_name}")
